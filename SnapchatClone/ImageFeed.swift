...
func addPost(postImage: UIImage, thread: String, username: String) {
    ...
    // YOUR CODE HERE
    dbRef.child(firReadPostsNode).childByAutoId().setValue(postDict)
            ...
            if let posts = snapshot.value as? [String:AnyObject] {
                user.getReadPostIDs(completion: { (ids) in
                    for postKey in posts.keys {
                        if let valuesDict = posts[postKey] as? [String:String] {
                            let newPost = Post(id: postKey, username: valuesDict["username"] ?? "", postImagePath: valuesDict["imagePath"] ?? "", thread: valuesDict["thread"] ?? "" , dateString: valuesDict["date"] ?? "", read: ids.contains((postKey)))
