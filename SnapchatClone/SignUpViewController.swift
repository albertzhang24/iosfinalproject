            ...
            Auth.auth().createUser(withEmail: email, password: password) { (user, error) in
                if error == nil {
                    ...
                    changeRequest?.commitChanges(completion: { (error) in
                        let alertController = UIAlertController(title: "Success", message: "Account successfully created!", preferredStyle: .alert)
                        let successAction = UIAlertAction(title: "OK", style: .cancel, handler: { (action) in
